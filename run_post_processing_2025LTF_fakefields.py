...
def main():
    ...
    * This will only work with katydid files that have track/event objects in the trees.
    ...
    arg(
        ...
        type=float,
        default=None,
        help="Fake fields to include in post-processing. If omitted, use the plain aid_### directory.",
    )
...
class PostProcessing:
    def __init__(
        ...
        self.identity_cols = ["run_id", "file_id", "analysis_variant"]
        self.physical_file_cols = ["run_id", "file_id"]
        ...
        if self.stage == -1:
            ...
            if missing_mask.any():
                ...
                # Define join keys. The fake-field variants share run_id/file_id,
                # so analysis_variant is required to avoid duplicate/cross joins.
                join_keys = self.identity_cols
    ...
    def get_experiment_files(self):
        ...
        for run_id in self.run_ids:
            for fake_field in self.get_fake_field_list():
                base, offline = self.build_file_df_path(run_id, fake_field=fake_field)
                variant = self.analysis_variant(fake_field)
        ...
        if missing_base:
            msg = "\n".join(
                f"  rid {p['run_id']}, variant {p['analysis_variant']}: missing {p['base']}"
        ...
        if has_offline and missing_offline:
            msg_has = "\n".join(
                f"  rid {p['run_id']}, variant {p['analysis_variant']}: {p['offline']}"
                for p in has_offline
            )
            msg_missing = "\n".join(
                f"  rid {p['run_id']}, variant {p['analysis_variant']}: expected {p['offline']}"
                for p in missing_offline
            )

            raise RuntimeError(
                "Mixed offline-monitor CSV state for stage 0.\n\n"
                "Some variants have *_with_offline_mon.csv:\n"
                f"{msg_has}\n\n"
                "Some variants do not:\n"
                f"{msg_missing}\n\n"
                "Either generate offline-monitor CSVs for all variants, "
        ...
        for p in paths:
            ...
            # Preserve the physical file identity, but add a unique analysis identity
            # for each fake-field replica.
            file_df["fake_field"] = p["fake_field"]
            file_df["fake_field_label"] = (
                None if p["fake_field"] is None
                else f"ff_{self.format_fake_field(p['fake_field'])}"
            )
            file_df["analysis_variant"] = p["analysis_variant"]
        ...
        # Unique row id for post-processing Slurm stage 1. This intentionally
        # differs from the physical file_id, which is duplicated across fake fields.
        root_files_df["pp_file_id"] = np.arange(len(root_files_df), dtype=int)

        duplicate_identity = root_files_df.duplicated(self.identity_cols)
        if duplicate_identity.any():
            print(root_files_df.loc[duplicate_identity, self.identity_cols + ["root_file_path"]])
            raise RuntimeError(f"Duplicate post-processing identity rows: {self.identity_cols}")

        return root_files_df

    def build_file_df_path(self, run_id, fake_field=None):
        base_path = Path("/data/raid2/eliza4/he6_cres/katydid_analysis/root_files")
        rid_ai_dir = base_path / f"rid_{run_id:04d}" / self.analysis_dir_name(fake_field)
    ...
    def process_tracks_and_events(self):
        # Stage-1 jobs split on pp_file_id when present. This avoids collisions when
        # multiple fake-field variants share the same physical run_id/file_id.
        job_id_col = self.get_job_id_col()
        root_files_df_chunk = self.root_files_df[
            self.root_files_df[job_id_col].astype(int) == int(self.file_id)
        ]
        if len(root_files_df_chunk) == 0:
            raise UserWarning(
                f"There is no {job_id_col} = {self.file_id} in aid = {self.analysis_id}"
            )
    ...
    def get_track_points_data_from_files(self, root_files_df):
        ...
        if not experiment_track_points_list:
            return pd.DataFrame()
    ...
    def build_bulk_track_params_for_single_file(self, root_files_df_row):
        ...
        tracks_df["analysis_id"] = root_files_df_row.get("analysis_id", self.analysis_id)
        tracks_df["analysis_variant"] = root_files_df_row.get("analysis_variant", self.analysis_variant())
        tracks_df["fake_field"] = root_files_df_row.get("fake_field", np.nan)
        tracks_df["fake_field_label"] = root_files_df_row.get("fake_field_label", None)
        tracks_df["pp_file_id"] = root_files_df_row.get("pp_file_id", root_files_df_row["file_id"])
        tracks_df["measured_field"] = root_files_df_row.get("measured_field", np.nan)
    ...
    def build_track_points_for_single_file(self, root_files_df_row):
        ...
        track_points_df["analysis_id"] = root_files_df_row.get("analysis_id", self.analysis_id)
        track_points_df["analysis_variant"] = root_files_df_row.get("analysis_variant", self.analysis_variant())
        track_points_df["fake_field"] = root_files_df_row.get("fake_field", np.nan)
        track_points_df["fake_field_label"] = root_files_df_row.get("fake_field_label", None)
        track_points_df["pp_file_id"] = root_files_df_row.get("pp_file_id", root_files_df_row["file_id"])
        track_points_df["measured_field"] = root_files_df_row.get("measured_field", np.nan)
    ...
    def add_env_data(self, root_files_df):
        ...
        # Preserve the measured NMR field, then use the fake field as the
        # analysis field for fake-field variants. This keeps downstream code
        # that expects a column named "field" working with fake-field scans.
        root_files_df["measured_field"] = root_files_df["field"]
        if "fake_field" in root_files_df.columns:
            fake_mask = root_files_df["fake_field"].notna()
            root_files_df.loc[fake_mask, "field"] = root_files_df.loc[fake_mask, "fake_field"]
        elif "true_field" in root_files_df.columns:
    ...
    def add_arduino_monitor_rate(self, root_files_df):
        ...
        for rid, root_files_df_gb in root_files_df.groupby(["run_id"]):
            ...
            for fid, file_path in root_files_df_gb.groupby("file_id"):
                ...
                # A physical file can appear multiple times for fake-field variants.
                # Environmental quantities are identical for those replicas.
                file_time = file_path.utc_time.iloc[0]

                # Get arduino_monitor_rate during run.
                arduino_monitor_rate = self.get_nearest(monitor_log, file_time).rate
        ...
        for rid, root_files_df_gb in root_files_df.groupby(["run_id"]):
            ...
            if field_log.empty:
                ...
                for fid, file_path in root_files_df_gb.groupby("file_id"):
                    ...
                    # A physical file can appear multiple times for fake-field variants.
                    # The measured NMR value is identical for those replicas.
                    file_time = file_path.utc_time.iloc[0]

                    # Get field during second of data
                    field = self.get_nearest(field_log, file_time).field

                    # Now get the nearest rate for each file_id and fill those in!! Then this gets joined with the whole table.
                    condition = (root_files_df["run_id"] == rid) & (
                        root_files_df["file_id"] == fid
                    )
                    root_files_df.loc[condition, "field"] = field
        ...
        for rid, root_files_df_gb in root_files_df.groupby(["run_id"]):
            ...
            if rga_log.empty:
                ...
                for fid, file_path in root_files_df_gb.groupby("file_id"):
                    ...
                    # A physical file can appear multiple times for fake-field variants.
                    # RGA values are identical for those replicas.
                    file_time = file_path.utc_time.iloc[0]

                    nearest_row = self.get_nearest(rga_log, file_time)
    ...
    def add_temps(self, root_files_df):
        ...
        for rid, root_files_df_gb in root_files_df.groupby(["run_id"]):
            ...
            if not rga_log.empty:
                ...
                for fid, file_path in root_files_df_gb.groupby("file_id"):
                    # A physical file can appear multiple times for fake-field variants.
                    # Temperature values are identical for those replicas.
                    file_time = file_path.utc_time.iloc[0]
        ...
        for rid, root_files_df_gb in root_files_df.groupby(["run_id"]):
            ...
            if voltage_log.empty:
                ...
                for fid, file_path in root_files_df_gb.groupby("file_id"):
                    ...
                    # A physical file can appear multiple times for fake-field variants.
                    # Voltage values are identical for those replicas.
                    file_time = file_path.utc_time.iloc[0]

                    # Get voltage during second of data
                    voltage = self.get_nearest(voltage_log, file_time).voltage
    ...
    def merge_csvs(self):
        job_id_col = self.get_job_id_col()
        if job_id_col in self.root_files_df.columns:
            track_ids = sorted(self.root_files_df[job_id_col].dropna().astype(int).unique())
        else:
            track_ids = list(range(self.num_files_tracks))

        track_ids = [i for i in track_ids if i < self.num_files_tracks]
        tracks_path_list = [
        ...
        if not all(tracks_path_exists):
            print(f"Not all requested tracks csvs are present for merging csvs.")
        ...
        if job_id_col in self.root_files_df.columns:
            point_ids = sorted(self.root_files_df[job_id_col].dropna().astype(int).unique())
        else:
            point_ids = list(range(self.num_files_points))

        point_ids = [i for i in point_ids if i < self.num_files_points]
        ...
        if not all(track_points_path_exists):
            print(f"Not all requested track points csvs are present for merging csvs.")

        # Filter the lists to include only the paths that exist
        track_points_path_list = [path for path in track_points_path_list if path.is_file()]

        if track_points_path_list:
            track_points_dfs = [
